...
namespace AppOwnsData.Services
{
    ...
    public class ConfigValidatorService
    {
        ...
        public ConfigValidatorService(IConfiguration configuration)
        {
        ...
        /// <returns></returns>
        public static string ValidateConfig(IOptions<AzureAd> azureAd, IOptions<PowerBI> powerBI, IConfiguration configuration)
        {
            string message = null;
            ...
            else if (string.IsNullOrWhiteSpace(configuration["AzureAd:ClientId"]))
            {
                message = "Client Id is not set in appsettings.json file";
            }
            else if (isAuthModeServicePrincipal && string.IsNullOrWhiteSpace(configuration["AzureAd:TenantId"]))
            {
            ...
            else if (string.IsNullOrWhiteSpace(configuration["PowerBI:WorkspaceId"]))
            {
            ...
            else if (isAuthModeServicePrincipal && string.IsNullOrWhiteSpace(configuration["AzureAd:ClientSecret"]))
            {
